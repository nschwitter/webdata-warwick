...
Using CSS selectors, we have already extracted specific information from a webpage by specifying what kind of element we want. Extracting links goes one step deeper, because hyperlinks are an *attribute* of the text. This means they are not directly visible on the page (as elements), but they are embedded within an element. 
...
We will now try this on the [Wikipedia list of countries](https://en.wikipedia.org/wiki/List_of_countries_by_population_(United_Nations)).We load the libraries again.
...
Before we will continue with our URLs, we will reduce them. We currently have a list of 231 countries. While this is complete, we do not want to bombard Wikipedia with that many requests later on. So for learning purposes, we will reduce the list of URLs which we will scrape. We will take every 13th element. This way, we will end up with around 20 countries to scrape with a varying population size (the countries are sorted by population size).
...
We now have a list of web pages that we want to scrape. Before we tackle the task of collecting information on all countries, we want to collect information on *one* country. We can then re-use this code and insert it into a loop to loop over the whole list of countries. 

What we want to collect in the following is the *anthem* of all countries. This will require us to apply what we have used in the previous sections. We will parse the first country page and extract the anthem of it. 

```{r}
page <- read_html(urls[1])
anthem <- html_text(html_nodes(page, ".anthem a"))
```

It works - we now know the anthem of China! We have collected the anthem of China, but this is not enough: We want the anthems of all countries. We have different options on how to run the same chunk of code across different units. 
...
We will save all anthems in the list object `anthem` which we create at the start, and then loop over our list. In the for-loop, we extract the information the way we did it before when we collected the anthem of China except that we now make use of `i`. 
...
for(i in 1:length(urls)){
  ...
  anthems[i] <- html_text(html_nodes(page, ".anthem a"))
}
anthems
```
...
The error occurs when we try to scrape French Guiana. What does the error message tell us?
...
html_text(html_nodes(page, ".anthem a"))
```

There seems to be a problem with the anthem - nothing is being extracted! Why is that? Let's check the [Wikipedia article for French Guiana](https://en.wikipedia.org//wiki/French_Guiana) manually to see what is going on.
...
for(i in 1:length(urls)){
  page <- read_html(urls[i])
  if (length(html_text(html_nodes(page, ".anthem a")))==0){
    anthems[i] <- NA
  }
  else{
    anthems[i] <- html_text(html_nodes(page, ".anthem a"))
...
There are other ways to loop over the pages. They might be more flexible, faster, and/or better legible. For example, you could define a function and then stick it into your `for`-loop. This is useful if you need to use the code of getting the anthem multiple times in multiple different places.
...
Instead of sticking the function into a for-loop, you can use it within an `apply()`-function.
...
Note we are using `sapply()`, one function from the `apply`-family. While the standard `apply()`-function works on anything that has dimensions in R (such as a matrix or data frame), `sapply()` can also be used on our list. 
...
Collecting data from multiple pages can often become a little bit of detective work. In the previous case, we could scrape all links from the first article which was a list of countries - we had an overview over all countries. In other cases, there might be a *view all*-button which allows us to display all information on one page so that we do not even need to loop through multiple pages. In other cases, we need to find some regularity in URLs.  
...
Doing that takes a little setup work first. First, we need to download binaries, start the driver and get the client object. We will open a `firefox` browser. If you use another browser, you can replace the value (`browser = c("chrome", "firefox", "phantomjs", "internet explorer")`). The port identifies your browser. You cannot open multiple instances on the same port. If you get the error message that a port is already in use, just change the port number. 

```{r}
library(RSelenium)
driver <- rsDriver(browser = "firefox", port = 4441L)
...
It is a little bit like magic: The browser clicked the button! We need to repeat this now. We know that 284 reviews have been written and 25 are displayed per page. Thus, there are 12 pages with information. We need to click the button 10 more times and can do so in a simple loop. Click on the browser and watch the magic happen!
...
1. Collect all user reviews for your favourite movie!
