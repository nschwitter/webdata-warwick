---
title: "Basic Web Scraping (with solutions)"
...
Next, we can read the page into R. To do that, we need to tell R its URL which we first save into (https://www.scrapethissite.com/pages/simple/) into the new object `url`. Next we parse the web page. We use the function `read_html()`.
...
Everything is there now, even lots of things we generally do not want to collect like scripts that were loaded in the background of the website. We can use the function `html_text()` to extract just the web page text.
...
## Extracting elements
The  method `html_elements()` allows the selection of specific elements of the HTML code. The documentation of the `html_elements()`command reveals that we need CSS selectors or XPath expressions to specify what we want to select. 
...
This already looks more structured and we have different nodes in our new element `selected_elements` - but we should get rid of the HTML tags. We can achieve this with the `html_text()` command.
...
We are almost there. We have 250 elements and we got rid of all HTML tags. We just still have some pre-processing to do. Pre-processing your collected data is an important step, particularly with web-scraped text data which tends to be messy as it is not primarily written for data analysis. What we want to do in the next step is to remove leading and trailing white spaces.  We use `twimws()` to do so. 
...
#SOLUTION
countries <- trimws(countries)
capitals <- trimws(html_text(html_elements(countries_website,".country-capital")))
population <- trimws(html_text(html_elements(countries_website,".country-population")))
area <- trimws(html_text(html_elements(countries_website,".country-area")))

countrydf <- data.frame(countries=countries,
                        capitals=capitals,
...
We will use [Wikipedia's list of countries (United Nations)](https://en.wikipedia.org/wiki/List_of_countries_by_population_(United_Nations)). We can apply the `html_table()` command to read in all tables in a document and convert them into a list of data frames. 
...
```{r} 
#SOLUTION
spacejam <- "https://www.imdb.com/title/tt0117705/fullcredits/"
spacejamhtml <- read_html(spacejam)

cast<-html_table(html_element(spacejamhtml, ".cast_list"))

#todo: preprocessing, rename columns, etc.

```

```{r} 
#SOLUTION
spacejamrev <- "https://www.imdb.com/title/tt0117705/reviews"
spacejamrevhtml <- read_html(spacejamrev)

reviews <- html_text(html_elements(spacejamrevhtml, ".review-container"))
