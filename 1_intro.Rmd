...
Before we dive into scraping data, we cover the basic of HTML. You only need a rudimentary understanding of HTML to scrape data from the web, but the more you know, the better you get at troubleshooting errors, working with exemptions and writing flexible solutions. 

## The Basics of HTML

HTML is a **H**yper**T**ext **M**arkup **L**anguage. The language uses **tags** to specify character and behaviour of text. A start and an end tag typically surrounding the text they are formatting with <> symbols.

Example: `<tagname>Text</tagname> `
 
Knowing HTML tags, you can also write your own website! Also, take a look at simplewebsite.html. I used all of the following HTML snippets in that document, so you can see how such HTML elements are being displayed in your browser. 
...
Hyperlinks are a common element on website: Linking to other websites is one of the core features of the web. Text content is turned into a link with the `<a>` tag (*anchor*), and the link address is specified as `href` attribute (*hyperreference*).

Example: `This is text <a href="https://www.scrapethissite.com/pages/simple/">with a link</a>.`
...
Classes are another special case of attributes used to specify a class for an HTML element. Multiple HTML elements can share the same class. We often encounter classes because they are frequently used for consistent formatting of similar content. 
...
CSS selectors can be further specified when they are combined. For example, you can select all elements A inside elements B by specifying `elementA elementB`. Selecting all elements A and all elements B is done by specifying `elementA, elementB`. For a list of CSS Selectors, check out [this collection](https://www.w3schools.com/cssref/css_selectors.asp) and if you want to practice CSS Selectors in a fun way, play [CSS Diner](https://flukeout.github.io/).
